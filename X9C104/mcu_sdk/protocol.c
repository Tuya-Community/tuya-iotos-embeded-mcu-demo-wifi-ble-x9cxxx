...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_RESISTANCE,resistance_value); //VALUE型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_resistance_handle
功能描述 : 针对DPID_RESISTANCE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_resistance_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long resistance;
    
    resistance = mcu_get_dp_download_value(value,length);
	  resistance_value=100-resistance;
	  F_TASK_X9C=1;
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_RESISTANCE,resistance);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    unsigned char ret;
    switch(dpid) {
        case DPID_RESISTANCE:
            //电阻值处理函数
            ret = dp_download_resistance_handle(value,length);
        break;
